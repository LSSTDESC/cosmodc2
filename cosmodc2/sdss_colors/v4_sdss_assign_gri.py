...
def assign_mstar_and_restframe_sdss_gri(mpeak_mock, upid_mock, mstar_mock, sfr_percentile_mock,
            host_halo_mvir_mock, redshift_mock, **kwargs):
    """ Primary function used to paint g, r, i SDSS flux onto UniverseMachine galaxies.
    ...
    ----------
    mpeak_mock : ndarray
        Numpy array of shape (ngals, ) storing peak halo mass in
        units of Msun assuming h=0.7
    ...
    -------
    new_mstar : ndarray
        Numpy array of shape (ngals, ) storing remapped stellar mass
    ...
    is_red_gr_mock : ndarray
        Numpy boolean array of shape (ngals, ) is True if the galaxy's
        restframe g-r color was drawn from the red sequence

    is_red_ri_mock : ndarray
        Numpy boolean array of shape (ngals, ) is True if the galaxy's
        restframe g-r color was drawn from the red sequence
    """
    ngals = len(upid_mock)
    ...
    new_mstar = lift_high_mass_mstar(
        mpeak_mock, mstar_mock, upid_mock,  redshift_mock)

    #  Calculate model values of Mr
    magr = magr_monte_carlo(new_mstar, redshift_mock, **kwargs)
    ...
    #  Redden the satellites of cluster-mass halos
    gr_mock, ri_mock, is_red_gr_mock, is_red_ri_mock = remap_cluster_satellite_gr_ri_color(
            upid_mock, new_mstar, host_halo_mvir_mock, magr, gr_mock, ri_mock,
            is_red_gr_mock, is_red_ri_mock)

    return new_mstar, magr, gr_mock, ri_mock, is_red_gr_mock, is_red_ri_mock
